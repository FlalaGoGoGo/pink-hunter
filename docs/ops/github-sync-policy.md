...
- Preferred command wrapper: `./scripts/ops_runner.sh sync-release "Sync latest product changes"`
- Example:
...
- The local `GitHub/` directory should contain only one export mirror: `GitHub/pink-hunter`.
- Local backup folders such as `pink-hunter.backup-*`, `pink-hunter.bak-*`, `pink-hunter.prev.*`, `pink-hunter.stale.*`, and `pink-hunter 2` are no longer part of the supported workflow.
- Local rollback retention is reduced to one compressed archive at `/Users/zhangziling/Documents/Project-Pink-Hunter-Archive/releases/pink-hunter-baseline-2026-03-cleanup.tar.gz`.
