...
- Card output spot-check: the relevant `public/data/trees.<region>.city.<slug>.v1.geojson`
...
5. Spot-check the relevant `public/data/trees.<region>.city.<slug>.v1.geojson` for card-ready `subtype_name` values in at least one city per affected group.
